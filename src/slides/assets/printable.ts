...
window.printable = (
  ...
  const addOverallMarkup = (interpreter: Interpreter, track: Track | null) => {
    const overallTitle =
      "Overall Rankings" +
    ...
    const overallTeams = interpreter.teams
      ...
      .reduce(
        ...
        [[], []] as [string[], typeof interpreter.teams],
      )[1]
      ...
      .map(
        (team, i) =>
          ...
          } <span class="supp">(${team.state}, Team ${team.number})</span></li>`,
      );
    ...
    contentMarkup += `<h2>${overallTitle}</h2><ul>${overallTeams.join("")}</ul>`;
  };
  ...
  const genOverall = (interpreter: Interpreter) => {
    ...
    overallData.forEach((track) => addOverallMarkup(interpreter, track));
  };

  const addEventMarkup = (events: [Event, Track | null][]) => {
    events.forEach(([event, track]) => {
      const eventPlaces = Math.min(
        ...
        event.medals ?? (track ? track.medals : event.tournament?.medals) ?? 0,
      ...
      const rankedTeams = event
        .placings!.filter(
          (p) =>
            (track ? p.team?.track === track : true) && !p.team?.exhibition,
        ...
        .filter((p, i) =>
          event.tournament?.reverseScoring
            ? i < eventPlaces
            : (track ? p.isolatedTrackPoints! : p.isolatedPoints!) <=
              eventPlaces,
      ...
      contentMarkup += `<h2>${eventName}</h2><ul>${rankedMarkup}</ul>`;
    });
  };
